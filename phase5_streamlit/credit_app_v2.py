...
def page_title(my_title="Application de scoring de crédits"):
    """
    Define header of the dashboard pages wwith logo 
    """
...
def load_data():
    ...
    return(X, X_rus_train, X_rus_test)
    # st.write(X)


def get_api_response(df):
    """
...
page = st.sidebar.selectbox("Choisissez la page à afficher",
    ["Accueil", "Comprendre les scores", "Score crédit", "Comparables"])

if page == "Accueil":
    page_title("Bienvenu sur l'application de scoring crédit")
    ...
    st.write("Cette page vous permet de rentrer les données de votre client.\
        Ensuite, vous avez juste à cliquer sur le bouton et le résultat du scoring est\
        retourné automatiquement.")
...
if page == "Comprendre les scores":
    ...
    st.write("En tant que chargé commercial, vous devez pouvoir expliquer \
        à nos clients comment fonctionne notre système de scoring. POur \
...
if page == "Score crédit":
    ...
    PREV_DAYS_DECISION_MIN = st.slider('PREV_DAYS_DECISION_MIN', 0, 3000, 1500)

    DAYS_EMPLOYED = st.slider('DAYS_EMPLOYED', 0, 20000, 2000)
    PREV_APP_CREDIT_PERC_MIN = st.slider('PREV_APP_CREDIT_PERC_MIN', 0.0, 3.0, 0.79)
    INSTAL_DPD_MAX = st.slider('INSTAL_DPD_MAX', 0, 2800, 20)
    AMT_CREDIT = st.slider('AMT_CREDIT', 30000, 3000000, 600000)
    DAYS_BIRTH = st.slider('DAYS_BIRTH', 4000, 30000, 16000)

    FLAG_OWN_CAR = st.select_slider('FLAG_OWN_CAR (0=No, 1=Yes)', options=[0, 1])
    CODE_GENDER = st.select_slider('Select sex (O=Female, 1=Male)', options=[0, 1])
    NAME_EDUCATION_TYPE_Higher_education = st.select_slider(
...
if page == 'Comparables':
    ...
    X, X_rus_train, X_rus_test = load_data()
